...
public class DetectionTest extends AbstractTest implements DetectionUtil {
    ...
    @Test
    public void i001() {
        var detection = Detector.detect(middle(y(-7.0), y(-1)), parseSequence("(-1 3, 5 -3, 5 -5, 3 -6, 5 -6, 5 -7, 1 -7, 3 -5, -1 -1, -1 0, 4 -5, 5 -4, -1 2, -1 3)"), true);
    ...
    public void i002() {
        ...
        var holeDetection = Detector.detect(middle(x(1), x(4)), hole, false);
...
//        println(toWKT(singletonList(x(-3)), singletonList(y(1)), parseSequence("(-3 5, -5 7, -7 5, -3 1, -3 2, -6 5, -5 6, -3 4, -3 5)"), GEOMETRY_FACTORY));
    ...
    @Test
    public void t02() {
        var detection = Detector.detect(middle(y(-7), y(-1)), parseSequence("(-5 7, -7 5, 3 -5, 1 -7, 7 -7, 7 -1, 5 -3, -5 7)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly(
                        "In.Y(2, (-1.0, -1.0, NaN))",
                        "Out.Y(3, null)",
    ...
    @Test
    public void t04() {
        var detection = Detector.detect(lower(y(7)), parseSequence("(-5 7, -7 5, 3 -5, 1 -7, 7 -7, 7 -1, 5 -3, -5 7)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .isEmpty();
    }
...
//        System.out.println(toWKT(singletonList(x(-4)), singletonList(y(4)), sequence, GEOMETRY_FACTORY));
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly("In.X(1, (-4.0, 6.0, NaN))", "Out.Y(2, (-6.0, 4.0, NaN))", "In.Y(5, (-5.0, 4.0, NaN))", "Out.X(6, (-4.0, 5.0, NaN))");
    ...
    @Test
    public void t06() {
        var detection = Detector.detect(upper(y(7)), parseSequence("(-5 7, -7 5, 3 -5, 1 -7, 7 -7, 7 -1, 5 -3, -5 7)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .isEmpty();
    }
    ...
    public void t07() {
        ...
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly("Out.Y(2, (-9.0, 0.0, NaN))", "In.Y(4, (-7.5, 0.0, NaN))");
    }

    @Test
    public void t08() {
        var detection = Detector.detect(middle(x(-1), x(5)), parseSequence("(-5 7, -7 5, 3 -5, 1 -7, 7 -7, 7 -1, 5 -3, -5 7)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly(
                        "In.X(2, (-1.0, -1.0, NaN))",
                        "Out.X(3, (5.0, -7.0, NaN))",
    ...
    @Test
    public void t10() {
        var detection = Detector.detect(middle(y(1), y(7)), parseSequence("(-5 7, -7 5, 3 -5, 1 -7, 7 -7, 7 -1, 5 -3, -5 7)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly(
                        "Out.Y(1, (-3.0, 1.0, NaN))",
                        "In.Y(7, (1.0, 1.0, NaN))"
...
//        println(toWKT(list(x(-4), x(-3)), list(y(-1), y(6)), parseSequence("(-3 5, -5 7, -7 5, -3 1, -3 2, -6 5, -5 6, -3 4, -3 5)"), GEOMETRY_FACTORY));
    ...
    @Test
    public void t12() {
        var detection = Detector.detect(middle(x(-6), x(-1)), parseSequence("(-5 6, -6 5, 4 -5, 5 -4, -5 6)"), true);
        assertThat(Ginsu.map(detection.events.getVector(), Event::toString))
                .containsExactly(
                        "Out.X(1, (-1.0, 0.0, NaN))",
                        "In.X(4, (-1.0, 2.0, NaN))"
